...
datablock StaticShapeData(DoorHall5A)
{
   category = "Doors";
   shapeFile = "~/data/doors/hall_door_5_a/door.dts";
};
function DoorHall5A::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorHall5A::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;

	if (%whichDoor.getPosition() $= "-169.984 64.6587 68.5926" ) {
			%can_open = 1;
	}
	else if (%whichDoor.getPosition() $= "-204.607 59.0127 68.5926" ) {
			%can_open = 1;
	}
...
function DoorHall5A::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorHall5A::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorHall5A::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORSOTHER::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorHall5A::DecrementSwing(%theDatablock, %whichDoor)
{
