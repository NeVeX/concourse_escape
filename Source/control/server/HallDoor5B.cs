...
datablock StaticShapeData(DoorHall5B)
{
   category = "Doors";
   shapeFile = "~/data/doors/hall_door_5_b/door.dts";
};
function DoorHall5B::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorHall5B::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;
	
	if (%whichDoor.getPosition() $= "-162.043 64.6587 68.5926" ) {
			%can_open = 1;
	}
	else if (%whichDoor.getPosition() $= "-196.711 59.0127 68.5926" ) {
			%can_open = 1;
	}
...
function DoorHall5B::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorHall5B::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorHall5B::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORSOTHER2::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorHall5B::DecrementSwing(%theDatablock, %whichDoor)
{
