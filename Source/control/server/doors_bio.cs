...
datablock StaticShapeData(DoorBio)
{
   category = "Doors";
   shapeFile = "~/data/doors/door_bio/door.dts";
};
function DoorBio::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorBio::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;

   	// FAR BIG ROOM ON THE RIGHT the bio door
	if (%whichDoor.getPosition() $= "295.548 16.728 68.7" ) {
...
function DoorBio::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorBio::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorBio::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORS::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorBio::DecrementSwing(%theDatablock, %whichDoor)
{
