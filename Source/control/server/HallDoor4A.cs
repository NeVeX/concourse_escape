...
datablock StaticShapeData(DoorHall4A)
{
   category = "Doors";
   shapeFile = "~/data/doors/hall_door_4_a/door.dts";
};
function DoorHall4A::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorHall4A::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;

	if (%whichDoor.getPosition() $= "-115.649 59.1818 68.5926" ) {
...
function DoorHall4A::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorHall4A::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorHall4A::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORSOTHER::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorHall4A::DecrementSwing(%theDatablock, %whichDoor)
{
