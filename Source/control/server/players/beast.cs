...
//============================================================================
$Play_Sound_Beast = getRandom(3);
// Load dts shapes and merge animations

Exec("~/data/models/avatars/beast/player.cs");
Exec("~/data/models/avatars/beast_white/player.cs");
Exec("~/data/models/avatars/beast_santa/player.cs");
...
datablock PlayerData(BeastAvatar)
{
   ...
   className = AIBeast;
   shapeFile = "~/data/models/avatars/beast/player1.dts";
   cameraMaxDist = 3;
   computeCRC = true;
   ...
   debrisShapeName = "~/data/models/avatars/beast/debris_player1.dts";
   debris = playerDebris;
...
datablock PlayerData(BeastAvatarWhite)
{
   renderFirstPerson = false;
   emap = true;

   className = AIBeast;
   shapeFile = "~/data/models/avatars/beast_white/player.dts";
   cameraMaxDist = 3;
   computeCRC = true;
   ...
   debrisShapeName = "~/data/models/avatars/beast_white/debris_player.dts";
   debris = playerDebris;
...
datablock PlayerData(BeastAvatarSanta)
{
   renderFirstPerson = false;
   emap = true;

   className = AIBeast;
   shapeFile = "~/data/models/avatars/beast_santa/player.dts";
   cameraMaxDist = 3;
   computeCRC = true;
   ...
   debrisShapeName = "~/data/models/avatars/beast_santa/debris_player.dts";
   debris = playerDebris;
...
datablock PlayerData(Goblin)
{
   ...
   className = AIBeast;
   shapeFile = "~/data/goblin/player.dts";
   cameraMaxDist = 3;
   computeCRC = true;
...
function AIBeast::onDisabled(%this,%obj,%state)
{
   ...
   // beast death sound
   if ( %this == 78 ) {
   	PlayMusic(Beast1_Death);
  	}
   else if ( %this == 76 ) {
   	PlayMusic(Beast4_Death);
  	}
   else if ( %this == 75 ) {
   	PlayMusic(Beast2_Death);
  	}
   else if ( %this == 77 ) {
   	PlayMusic(Beast3_Death);
  	}
  //if ( $Playing == 0 ) {	
	  	// duke victory sound
		if ( $Play_Sound_Beast % 3 == 0 ) {
		 		PlayMusic(Duke_Kill_1);
			}
		else if ( $Play_Sound_Beast % 3 == 1 ) {
		 		PlayMusic(Duke_Kill_2);
			}
		else if ( $Play_Sound_Beast % 3 == 2 ) {
		 		PlayMusic(Duke_Kill_3);
			}
		 	$Play_Sound_Beast = $Play_Sound_Beast + 1;
	 //}
   // Release the main weapon trigger
   %obj.setImageTrigger(0,false);

   // Schedule corpse removal.  Just keeping the place clean.
   %obj.schedule(1500, "startFade", 1500, 0, true);
   %obj.schedule(2200, "delete");
   echo("THE BEEAST DEAD IS "@%obj);
   //%obj.schedule(10, "delete");
   $Beasts_Left--;
   if ( $Beasts_Left < 3 && $How_Many_Spawns < $How_Many_Spawns_Allowed) {
   	schedule(1000, 0, "SpawnTriggers_Survive");
   	echo("beasts_left is "@$Beasts_Left@" and how many spawns is "@$How_Many_Spawns);
   	echo("how many spawns allowed is "@$How_Many_Spawns_Allowed);
   	$How_Many_Spawns++;
  }
