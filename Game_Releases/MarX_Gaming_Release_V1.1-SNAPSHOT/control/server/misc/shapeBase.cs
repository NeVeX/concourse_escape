...
function ShapeBase::setDamageDt(%this, %damageAmount, %damageType)
{
   ...
   if (%obj.getState() !$= "Dead" || $Health_Left > 0) {
...
function ShapeBase::incInventory(%this,%data,%amount)
{
   ...
   if ( (%data.getName()$="Gold") || (%data.getName()$="Copper") || (%data.getName()$="Silver") )
   {
     ...
     $ReSpawn = ""@$Human.getPosition()@" 1 0 0 0";
   }
   if (%total < %max) {
      if (%total + %amount > %max)
         %amount = %max - %total;
      ...
      if ( %this == $Human ) {
      	// only update the screen iff the player has picked up the ammo with the associated gun in hand
      	if ( $Human.getMountedImage(0) == 33 && %data.getName()$= "M16Ammo" ) {
      		// m16 ammo pick up;
      		Ammobox.SetValue($Ammo_Left_now + %amount);
     		 }
     		else if ( $Human.getMountedImage(0) == 52 && %data.getName()$= "CrossbowAmmo" ) {
      		// crossbow ammo pick up;
      		Ammobox.SetValue($Ammo_Left_now + %amount);
     		 }
     		else if ( $Human.getMountedImage(0) == 19 && %data.getName()$= "TommyGunAmmo" ) {
      		// crossbow ammo pick up;
      		Ammobox.SetValue($Ammo_Left_now + %amount);
     		 }
     		}
