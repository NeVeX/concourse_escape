...
datablock StaticShapeData(DoorYellow)
{
   category = "Doors";
   shapeFile = "~/data/doors/door_yellow/door.dts";
};
function DoorYellow::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorYellow::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;
	
 	// BLUE BLOCK DOOR
	if (%whichDoor.getPosition() $= "-184.112 107.512 68.7" ) {
...
function DoorYellow::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorYellow::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorYellow::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORS::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorYellow::DecrementSwing(%theDatablock, %whichDoor)
{
