...
datablock StaticShapeData(DoorBankEntrance)
{
   category = "Doors";
   shapeFile = "~/data/doors/door_bank/door.dts";
};
function DoorBankEntrance::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorBankEntrance::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;
	
 	
   	// BANK ENTRANCE
	if (%whichDoor.getPosition() $= "-207.822 18.963 68.7" ) {
...
function DoorBankEntrance::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorBankEntrance::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorBankEntrance::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORS::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorBankEntrance::DecrementSwing(%theDatablock, %whichDoor)
{
