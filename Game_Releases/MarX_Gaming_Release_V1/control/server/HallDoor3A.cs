...
datablock StaticShapeData(DoorHall3A)
{
   category = "Doors";
   shapeFile = "~/data/doors/hall_door_3_a/door.dts";
};
function DoorHall3A::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorHall3A::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;

	if (%whichDoor.getPosition() $= "8.50097 93.1032 68.5926" ) {
	...
	else if (%whichDoor.getPosition() $= "-31.9717 84.408 68.5926" ) {
...
function DoorHall3A::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorHall3A::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorHall3A::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORSOTHER::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorHall3A::DecrementSwing(%theDatablock, %whichDoor)
{
