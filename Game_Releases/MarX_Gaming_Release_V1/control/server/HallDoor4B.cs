...
datablock StaticShapeData(DoorHall4B)
{
   category = "Doors";
   shapeFile = "~/data/doors/hall_door_4_b/door.dts";
};
function DoorHall4B::OnAdd(%theDatablock, %whichDoor)
{
...
function DoorHall4B::Operate(%theDatablock, %whichDoor) {
	%can_open = 0;
	
	if (%whichDoor.getPosition() $= "-107.643 59.1818 68.5926" ) {
...
function DoorHall4B::StartOpenSwing(%theDatablock, %whichDoor)
{
...
function DoorHall4B::IncrementSwing(%theDatablock, %whichDoor)
{
...
function DoorHall4B::StartCloseSwing(%theDatablock, %whichDoor)
{
  %theDatablock.schedule($DOORSOTHER2::CLOSE_TIMER,"DecrementSwing", %whichDoor);
  PlayMusic(DoorClosing);
}

function DoorHall4B::DecrementSwing(%theDatablock, %whichDoor)
{
